    ...
    if (lineCount) {
        ...
        for (int i = 0; i<lineCount; i++) {
            ...
            CGRect lineFrame = CGRectMake(baselineOrigin.x,
                                          ...
                                          lineWidth,
                                          ascent+descent+linegap+[XDSReadConfig shareInstance].lineSpace);
...
+(NSArray *)parserRectsWithPoint:(CGPoint)point
    ...
    if (lineCount) {
        ...
        for (int i = 0; i<lineCount; i++){
            ...
            CGFloat xStart;
            CGFloat xEnd;
            NSRange drawRange = [self selectRange:NSMakeRange((*selectRange).location, (*selectRange).length)
                                        lineRange:NSMakeRange(stringRange.location, stringRange.length)];
