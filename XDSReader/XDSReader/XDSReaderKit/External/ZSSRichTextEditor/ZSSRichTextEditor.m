...
//    self.editorView.keyboardDisplayRequiresUserAction = NO;
    ...
    if(DEBUG){
        JSContext *ctx = [self.editorView valueForKeyPath:@"documentView.webView.mainFrame.javaScriptContext"];
...
//    if (action == @selector(menuCopy:) || action == @selector(menuNote:) || action == @selector(menuShare:)) {
//        return YES;
//    }
//
...
- (void)setUnderline {
    ...
    NSString *result = [self.editorView stringByEvaluatingJavaScriptFromString:trigger];
    NSLog(@"result = %@", result);
}
