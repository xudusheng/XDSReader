...
@interface LPPHaloButton ()


@end
@implementation LPPHaloButton
...
- (void)createHaloView{
    self.imageView = ({
        CGRect frame = CGRectMake(kSpace_lpp_10,
                                  kSpace_lpp_10,
                                  CGRectGetWidth(self.frame) - kSpace_lpp_20,
                                  CGRectGetHeight(self.frame) - kSpace_lpp_20);
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:frame];
        // 圆角
        imageView.layer.cornerRadius = (CGRectGetWidth(self.frame) - kSpace_lpp_20) / 2;
...
- (void)setSelected:(BOOL)selected{
    if (selected) {
        if (self.selectImage) {
            ...
            self.imageView.layer.borderColor = TEXT_COLOR_LPP_2.CGColor;
